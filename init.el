...
(when (and running-interactively
  ...
  (defun ido-imenu ()
    "Use `ido' to select an item for `imenu'."
    (interactive)
    (require 'thingatpt)
    (require 'imenu)
    (imenu--make-index-alist)
    (let (items)
      (cl-labels ((collect-cars (alist)
                    (dolist (key alist)
                      (cond
                       ((and (listp key)
                             (imenu--subalist-p key))
                        (collect-cars key))
                       ((listp key)
                        (add-to-list 'items (car key)))))))
        (and imenu--index-alist
             (collect-cars imenu--index-alist)))
      (let ((sap (thing-at-point 'symbol)))
        (imenu (ido-completing-read "Item? " items nil t (and (member sap items) sap)))))))
...
(global-defkey "<f12>" (if (fboundp 'ido-imenu) 'ido-imenu 'imenu))
...

;;; Calc
(add-hook 'calc-load-hook
          (lambda ()
            (setq calc-full-mode t)
            (setq calc-display-trail nil)
            (setq calc-group-char "\s")
