...
;;; Copyright (C) 1992, 94, 95, 97, 2001 Noah S. Friedman
...
;;; $Id: escreen.el,v 1.16 2002/03/07 09:42:10 friedman Exp $
...
;; If you are using Emacs 18 or Emacs 19, you may have trouble loading this
;; program because of the customs syntax officially introduced in Emacs 20.
;; In that case, first load cust-stub.el, available from
;;
...
;; Todo:
;;   * per-frame screen configurations
;;   * symbolic names for screens (a la wicos.el)
...
;;     (this may be impossible to do sanely.)
...
;;
;;  (screen-number #<window-configuration>
...
(defun escreen-save-current-screen-configuration ()
  (let ((screen-data (escreen-screen-defined))
    ...
    (if screen-data
        (setcdr screen-data (escreen-save-screen-map))
      (setq new-alist-member (cons escreen-current-screen-number
                                   (escreen-save-screen-map)))
...
(defun escreen-save-user-data ()
  (mapcar (function (lambda (pair) (funcall (car pair))))
...
(defun escreen-window-edges (&rest args)
  (apply (if (fboundp 'window-edges)
             'window-edges
           ;; For XEmacs 19.13 and later.
           'window-pixel-edges)
         args))

(defun escreen-force-mode-line-update (&optional allp)
  (if (fboundp 'force-mode-line-update)
      (force-mode-line-update allp)
    ;; Emacs 18 doesn't have force-mode-line-update
    (and all (save-excursion (set-buffer (other-buffer))))
    (set-buffer-modified-p (buffer-modified-p))))

;; Some versions of emacs 18 did not have `member'.
